class EventDateValidator < ActiveModel::Validator
  ...
  def valid_beginning_year_event(event)
    ...
    event.errors[:beginning_date] << I18n.t('events.errors.year_used') if events_beginning.present?
  end
  ...
  def valid_end_year_event(event)
    ...
    event.errors[:end_date] << I18n.t('events.errors.year_used') if events_end.present?
  end
