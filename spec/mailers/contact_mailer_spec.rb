...
RSpec.describe ContactMailer, type: :mailer do
  ...
  describe 'send unregistered email' do
    let(:contact) { create(:contact) }
    let(:mail) { described_class.with(contacts: contact).unregistered_contact.deliver_now }

    it 'renders the subject' do
      expect(mail.subject).to eq(I18n.t('mail.unregistered.subject'))
    end
  ...
  describe 'send updated email' do
    let(:contact) { create(:contact) }
    let(:mail) { described_class.with(contacts: contact).self_update_contact.deliver_now }
